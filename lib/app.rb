...
def client_function
  client_login_function = login_twitter

return client_login_function
...
def like_recent_message
  ...
  client_function.search("#bonjour_monde", result_type: "recent").take(25).collect do |i|
  client_function.favorite(i.id)
...
def follow_recent
    ...
    client_function.search("#bonjour_monde", result_type: "recent").take(20).collect{|tweet| client.follow tweet.user}
    end
  end
end
